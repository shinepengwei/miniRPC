...
TcpEntity::TcpEntity(boost::asio::io_service &io):ios(io),_sock(new boost::asio::ip::tcp::socket(ios)){}

void TcpEntity::sendMessage(std::string str){
	std::cout<<"sending:"<<str<<std::endl;
	_sock->async_write_some(boost::asio::buffer(str.c_str(),strlen(str.c_str())),boost::bind(&TcpEntity::write_handler,this,boost::asio::placeholders::error));
}

void TcpEntity::write_handler(const boost::system::error_code &){
	std::cout<<"send msg complete!"<<std::endl;
}

void TcpEntity::read_handler(const boost::system::error_code& ec,boost::shared_ptr<std::vector<char>> str){
	if (ec){
	...
	_sock->async_read_some(boost::asio::buffer(*str2),boost::bind(&TcpEntity::read_handler,this,boost::asio::placeholders::error,str2));
}
//rpc server service
void TcpEntity::addService(google::protobuf::Service *serv){
...
void TcpEntity::deal_rpc_data(boost::shared_ptr<std::vector<char>> str){
...
TcpServer::TcpServer(boost::asio::io_service & io):TcpEntity(io),acceptor(ios,boost::asio::ip::tcp::endpoint(boost::asio::ip::tcp::v4(),6688)){
	_start();
}
...
TcpClient::TcpClient(boost::asio::io_service & io):TcpEntity(io),ep(ip::address::from_string("127.0.0.1"),6688){
